...
class Http4sEmberClient(client: Client[Task], closeFn: () => Unit)(implicit
    runtime: Runtime[Any],
    dispatcher: Dispatcher[Task]
) extends SdkAsyncHttpClient {

  import Http4sEmberClient._
...
object Http4sEmberClient {
  ...
  private[http4sEmber] case class Http4sEmberClientBuilder(
      customization: EmberClientBuilder[Task] => EmberClientBuilder[Task] =
        identity
  )(implicit runtime: Runtime[Any])
      extends SdkAsyncHttpClient.Builder[Http4sEmberClientBuilder] {

    def withRuntime(runtime: Runtime[Any]): Http4sEmberClientBuilder =
      copy()(runtime)
...
//    }
  ...
  private[http4sEmber] def builder(): Http4sEmberClientBuilder =
    Http4sEmberClientBuilder()(Runtime.default)

  val default: ZLayer[Any, Throwable, HttpClient] = customized(identity)

  def customized(
      f: EmberClientBuilder[Task] => EmberClientBuilder[Task]
  ): ZLayer[Any, Throwable, HttpClient] =
    ZLayer.scoped {
      ZIO.runtime.flatMap { implicit runtime: Runtime[Any] =>
        Http4sEmberClient
          .Http4sEmberClientBuilder(f)
          .toScoped
          .map { c =>
            new HttpClient {
